...
class Outputs(BaseModel):
    recommendation: str = Field(..., description="Plain-text event recommendation")
...
def _impl(body) -> Outputs:  # noqa: ANN001
    """Bridge A2A message → `Inputs` → `suggest_event`.
    ...
    recommendation = suggest_event(prefs.model_dump())
...
async def tasks_send(body: dict):
    ...
    event-preference JSON.  Returns a Task-like dict with the
